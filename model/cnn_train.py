...
def train_cnn_model(symbol_name, timeframe):
    # Symbol and folder name
    symbol_timeframe = f'{symbol_name}_{timeframe}'
    ...
    model.add(tf.keras.layers.Conv1D(filters=64, kernel_size=2, activation='relu', input_shape=(initial_window_size, 1)))
    ...
    model_checkpoint = ModelCheckpoint(os.path.join(folder_name, f"{symbol_name}_cnn_model.keras"), save_best_only=True)
    ...
    model.save(os.path.join(folder_name, f"{symbol_name}_cnn_model.keras"))

    # Save the scaler and data with CNN prefix
    joblib.dump(scaler, os.path.join(folder_name, f"{symbol_name}_cnn_scaler.pkl"))
    np.save(os.path.join(folder_name, f"{symbol_name}_cnn_x_train.npy"), x_train)
    np.save(os.path.join(folder_name, f"{symbol_name}_cnn_y_train.npy"), y_train)
    np.save(os.path.join(folder_name, f"{symbol_name}_cnn_x_test.npy"), x_test)
    np.save(os.path.join(folder_name, f"{symbol_name}_cnn_y_test.npy"), y_test)
